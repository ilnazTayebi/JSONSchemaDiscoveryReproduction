...
# Create an account
curl $host:$port/api/register -X POST -H 'Content-Type: application/json' -d '{"username": "test", "email": "test@test.de", "password": "test123456"}' > /usr/bin/reproresults/info.txt
# Login and get back the Token
curl $host:$port/api/login -X POST -H 'Content-Type: application/json' -d '{"email": "test@test.de", "password": "test123456"}' > /usr/bin/reproresults/token.txt
